...
const Detail = (props) => {
  ...
  const { stomp, user, setIsLoading, selectedPost, setSelectedPost } =
    useContext(Context);
  ...
  loadPost = useCallback(async () => {
    ...
    try {
      ...
      const url = `http://localhost:8005/posts/${id}`;
  ...
  loadPostReaction = async () => {
    ...
    try {
      setIsLoading(true);
      const url = "http://localhost:8005/posts";
  ...
  const loadUserFollower = async () => {
    ...
    try {
      setIsLoading(true);
      const url = `http://localhost:8002//users/${id}/followers`;
      const response = await axios.post(url, {
        followerId: id,
  ...
  const viewProfile = () => {
    ...
    history.push(`/users/${post.post_created_by}`);
  };

  const sendCustomMessage = ({ message, type, receiverId }) => {
    const receiverID = receiverId;
    const customType = type;
    const receiverType = stomp.RECEIVER_TYPE.USER;
    const customData = { message };
    const customMessage = new stomp.CustomMessage(
      receiverID,
      receiverType,
      customType,
      customData
    ...
    stomp.sendCustomMessage(customMessage).then(
      (message) => {},
      (error) => {}
    );
  };
  ...
  const updateNumberOfFollowers = async (numberOfFollowers) => {
    const url = "http://localhost:8002/users/followers";
  ...
  const like = async () => {
    const url = "http://localhost:8005/posts/in";
        ...
        <div className="post-detail__main">
          <div className="post-detail__main-left">
            <video controls width="100%">
              <source src={`http://localhost:8080${post?.post_content}`} alt={`${post?.post_created_by} - ${post?.post_created_date}`} type="video/webm" />
              <source
                src={`http://localhost:8080${post?.post_content}`} alt={`${post?.post_created_by} - ${post?.post_created_date}`}
                type="video/mp4"
              />
          ...
          <div className="post-detail__main-right">
            <div className="post-detail__creator">
              <img
                src={`http://localhost:8005${post?.user_avatar}`}
                alt={user.avatar}
              />
