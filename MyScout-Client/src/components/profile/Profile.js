...
const Profile = (props) => {
  ...
  const { stomp, user, setIsLoading, hasNewPost, setHasNewPost } = useContext(Context);
  ...
  const sendCustomMessage = ({ message, type, receiverId }) => {
    ...
    const customMessage = new stomp.CustomMessage(receiverID, receiverType, customType, customData);
