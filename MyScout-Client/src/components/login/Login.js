...
const Login = (props) => {
  ...
  const { setUser, setIsLoading, stomp } = useContext(Context);
  ...
  const loginstomp = async (user) => {
    const authKey = `${process.env.REACT_APP_stomp_AUTH_KEY}`;
    return await stomp.login(user.id, authKey);
  };
  ...
  const login = async () => {
    ...
    if (isUserCredentialsValid(email, password)) {
      ...
      const stompAccount = await loginstomp({id: authenticatedUser.data.id});
      if (stompAccount) {
    ...
    <div className="login__container">
      <div className="login__welcome">
        <div className="login__logo">
          <strong>MyScout</strong>
        </div>
        <p>Share yout talents</p>
      </div>
