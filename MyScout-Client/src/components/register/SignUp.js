...
function SignUp(props) {
  ...
  const { stomp, setIsLoading } = useContext(Context);
  ...
  const createstompAccount = async ({ userUuid, fullname, avatar }) => {
    const authKey = `${process.env.REACT_APP_stomp_AUTH_KEY}`;
    const user = new stomp.User(userUuid);
    user.setName(fullname);
    user.setAvatar(avatar);
    return await stomp.createUser(user, authKey);
  };
  ...
  const signup = async () => {
    ...
    if (isSignupValid({ fullname, email, password, confirmPassword })) {
      ...
      if (response && response.data.message) { 
        ...
        const createdAccount = await createstompAccount({ userUuid, fullname, avatar: `http://localhost:9000/${response.data.avatar}` });
        if (createdAccount) {
